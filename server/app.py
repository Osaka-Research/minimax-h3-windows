...
Basic (any username, password = API_KEY); the two pipeline-facing routes
...
<h2>Jobs</h2>
<table>
<tr><th>ID</th><th>Prompt</th><th>Status</th><th>Created</th><th>Result</th></tr>
{% for job in jobs %}
<tr>
  <td>{{ job.id[:8] }}</td>
  <td>{{ job.prompt[:200] }}</td>
  <td>{{ job.status }}</td>
...
def next_job():
    with get_db() as conn:
        ...
        if row is None:
            return "", 204
        conn.execute("UPDATE jobs SET status='in_progress', claimed_at=datetime('now') WHERE id=?", (row["id"],))
...
def upload_result(job_id):
    ...
    with get_db() as conn:
        conn.execute(
            "UPDATE jobs SET status='done', done_at=datetime('now'), video_filename=? WHERE id=?",
            (filename, job_id),
        )
